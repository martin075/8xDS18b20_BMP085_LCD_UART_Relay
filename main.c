...
*
* 	
*	Host configuration Atmega328, 8MHz
...
*	PB4  LCD_DATA_RS
*	PB5  LCD_DATA_R/W
*	PB6  LCD_DATA_E 	-teraz PC3
*
*	PD0  RxD, 19200,8,n,1
*	PD1  TxD
*	PD6  1-Wire Data, external 15K, 4k7 pullup
*	PD5  PWM LCD Backlight (OC2)
*
*	PB7  momentary MODE button - teraz PC2
...
#define BAUD 19200
#define TMR_TICK 200L	// 200Hz = 5ms, 8MHz for osc of atmega
...
#define DEF_BACKLIGHT 220	//default value of backlight
...
#define MODE_altitude 6		// altitude correction for relative barometric pressure
...
#define BTN_MODE ((~PINC & (1 << PC2)) >> PC2)
...
int32_t gAltitudeCorr; // altitude correction for realative barometric pressure
...
#error "Unknown target device"
	...
	DDRC &= ~(1 << PC2);	// set for input - button 1 - MODE
...
#elif defined(__AVR_ATmega16__)
#define PWM_OUT OCR2    // PWM register
#define TIMSK1 TIMSK
...
#error "Unknown target device"
	...
	// announce our birth
	uart_puts_P( "DS18x20,BMP180 Logger v" MAJOR_VER "." MINOR_VER "\r");
	lcd_gotoxy( 0, 0);
		...
		case MODE_BACKLIGHT:	// displaying interval change   *********************
			{
				...
				lcd_clrscr();	// ack button press
				while( BTN_MODE);	// wait until switch released
				mode = MODE_altitude;  //odkaz na dalsi
				break;	// next case
			}
		case MODE_altitude:	// offset relative barometric pressure in hPa
			{
				lcd_clrscr();
				lcd_puts_P( "Altitude corr hPa\n"); // value hPa relative to sea level
				sprintf( gBuffer, "%4i", gAltitudeCorr);
				lcd_puts( gBuffer);
				while( ! BTN_MODE)
				{
					while( BTN_UP)	// increase altitude
					{
						//if( gAltitudeCorr < 255)
						{
							gAltitudeCorr++;
							lcd_gotoxy( 0,1);
							sprintf( gBuffer, "%4i", gAltitudeCorr);
					...
					while( BTN_DOWN)	// decrease altitude
					{
						if( gAltitudeCorr > 0)
						{
							gAltitudeCorr--;
				...
				lcd_clrscr();	// ack button press
				mode = MODE_SENSOR;  //odkaz na zaciatok
				while( BTN_MODE);	// wait until switch released
				break;	// next case
			}	
				//while( BTN_MODE);	// wait until switch released
...
void display_temperatures( uint8_t num_sensors, uint8_t page)
{
	...
	//podmienka so zobrazenim sipky hore/dole/rovno -kod podla displeja C5hore/C6dole/C7rovno, pre vonkajsiu teplotu
		...
		//get altitude - it is depend on barometric pressure
